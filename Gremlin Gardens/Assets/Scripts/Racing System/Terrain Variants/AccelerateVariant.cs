...
public class AccelerateVariant : TerrainVariant
{
    ...
    {
        return Mathf.Abs(-scale * Mathf.Pow(activeModule.timePassed - Mathf.Pow((speedModifier + gremlin.gremlin.getStat("Running")), 1/exponent), exponent) + ((speedModifier + gremlin.gremlin.getStat("Running")) * scale));
    }
}
